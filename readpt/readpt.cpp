...
vector<string> PDE2M{"present", "R/W", "U/S", "PWT", "PCD", "Accessed", "Dirty", "PS","Global", "", "", "", "PAT"};
// vector<string> PDE4M{"present", "R/W", "U/S", "PWT", "PCD", "Accesed", "Dirty", "PS-Page size", "Gloabl", "", "", "", "PAT", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""};
vector<string> PDE4K{"present", "R/W", "U/S", "PWT", "PCD", "Accessed", "", "PS-Page size", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""};
...
void readPDE4K(unsigned long content) {
    cout << "read PDE-4K" << endl;
    if (content & 1 == 0) {
        cout << "    not present" << endl;
        return ;
    }
    for (int i = 0;i < 32;i++) {
        if (PDE4K[i] != "" && (content & mask[i])) {
            cout << "    bit " << i << " " << PDE4K[i] << endl;
        }
        if (i == 1 && content & mask[1] == 0) {
            // 如果这里是0
            cout << "    bit " << i << " " << PDE4K[i] << " - R" << endl; 
        }
        if (i == 2 && content & mask[2] == 0) {
            // 如果这里是0
            cout << "    bit " << i << " " << PDE4K[i] << " - U" << endl; 
        }
    }
    cout << endl;
